...
export default function PaymentInOutNewEditForm({ currentPayment }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    formData.append('company', user?.company?._id);
    ...
    try {
      if (currentPayment) {
        const res = await axios.put(
          `${import.meta.env.VITE_BASE_URL}/${user?.company?._id}/payment/${currentPayment._id}`,
          formData
        );
        mutate();
        router.push(paths.dashboard.accounting['payment-in-out'].list);
        enqueueSnackbar(res?.data.message);
        reset();
      } else {
        const res = await axios.post(
          `${import.meta.env.VITE_BASE_URL}/${user?.company?._id}/payment`,
          formData
        );
        router.push(paths.dashboard.accounting['payment-in-out'].list);
        enqueueSnackbar(res?.data.message);
